...
class Follow_up extends Controller {
    ...
    public function search_pending_enq(Request $request) {
        $data['type'] = "search_pending_enq";
        $holiday = Data_model::retrive('minimum_days', '*', array('type' => 'holiday'), 'type');
        $h_day = $holiday[0]->days;
        $day_array = array(0 => 'Sunday', 1 => 'Monday', 2 => 'Tuesday', 3 => 'Wednesday', 4 => 'Thursday', 5 => 'Friday', 6 => 'Saturday');
        ...
        if ($this->role_id == 1) {
            $data['pending_enq'] = DB::table($this->foreign_table)
                    ->select($this->foreign_table . '.*', 'customer_master.*', 'quatation.quatation_id', 'quatation.quatation_no', 'quatation.follow_up_status', 'quatation.q_no')
                    ->join('customer_master', 'customer_master.customer_id', '=', $this->foreign_table . '.customer_id')
                    ->join('quatation', 'quatation.' . $this->foreign_id, '=', $this->foreign_table . '.' . $this->foreign_id)
//                    ->where('quatation.quatation_date', '=', $quatation_date)
                    ->Where('inquiry.order_status', '=', 0)
                    ->Where('inquiry.regret_status', '=', 0)
                    ->where('quatation.follow_up_status', 0)
                    ->orderBy('quatation.q_no', 'DESC')
//                    ->limit(100)
                    ->get();
        } else {
...
//                    ->limit(100)
    ...
    public function search_visit_done(Request $request) {
        $data['type'] = "search_visit_done";
        $holiday = Data_model::retrive('minimum_days', '*', array('type' => 'holiday'), 'type');
        $h_day = $holiday[0]->days;
        $day_array = array(0 => 'Sunday', 1 => 'Monday', 2 => 'Tuesday', 3 => 'Wednesday', 4 => 'Thursday', 5 => 'Friday', 6 => 'Saturday');
        $qdate = date('Y-m-d', strtotime($request->search_date));
        ...
        if ($this->role_id == 1) {
            $data['visit_done'] = DB::table($this->foreign_table)
                    ->select($this->foreign_table . '.*', 'customer_master.*', 'quatation.quatation_id', 'quatation.quatation_no', 'quatation.follow_up_status', 'quatation.q_no')
                    ->join('customer_master', 'customer_master.customer_id', '=', $this->foreign_table . '.customer_id')
                    ->join('quatation', 'quatation.' . $this->foreign_id, '=', $this->foreign_table . '.' . $this->foreign_id)
//                    ->where('quatation.quatation_date', '=', $quatation_date)
...
//                    ->limit(100)
    ...
    public function follow_up_add(Request $request) {
        ...
        $next_follow_time = (isset($request->next_follow_time)) ? $request->next_follow_time : NULL;
    ...
    public function document_add(Request $request) {
        ...
        $document_related_detail = (isset($request->document_related_detail)) ? $request->document_related_detail : NULL;
        ...
        $document_data = array(
            ...
            'document_related_detail' => $document_related_detail,
            'added_by' => $this->user_id);
