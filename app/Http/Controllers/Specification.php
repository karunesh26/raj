...
{
	public $table="specification_master";
	public $primary_id="specification_id";
	public $field = "specification";
	public $msgName = "Specification";
	public $view = "specification";
	public $controller = "Specification";
	public $module_name = "specification";
	public $utility;
	public $role_id;
	public $user_id;
	...
	public function insert(Request $request)
	{
		...
		$spe_name = implode("+++++",$request->input('spe_name'));
		$spe_value = implode("+++++",$request->input('spe_value'));
		...
		if ($request->hasFile('image')) 
		{
			...
			$path = 'external/specification_image/';
			if(!is_dir($path))
			{
				 mkdir($path, 0777, TRUE);
			}
			
			$file_nm= time().'.'.$file_ext;
			$file->move($path,$file_nm);
		...
		$data = array('sequence'=>$sequence,$this->field=>$name,'spe_name'=>$spe_name,'spe_value'=>$spe_value,'application'=>$application,'image'=>$file_nm);
	...
	public function update(Request $request)
	{
		/*for($i=1; $i<=28; $i++)
		{
			Data_model::restore($this->table,array('sequence'=>$i),array('specification_id'=>$i));
		}
		exit;*/
		...
		{
			$file_nm=$request->input('old_image');
		}
