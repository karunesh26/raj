...
class Quatation_product extends Controller
{
	public $table="quatation_product";
	public $primary_id="p_id";
	public $msgName = "Product";
	public $view = "quatation_product";
	public $controller = "Quatation_product";
	public $field = "name";
	...
	public function index()
	{
		...
		/* $data['result'] = Data_model::db_query("select `".$this->table."`.*,`power_supply_master`.power_supply_name from `".$this->table."` LEFT JOIN `power_supply_master` ON `power_supply_master`.power_supply_id = `".$this->table."`.power_supply_id Order By ".$this->field." asc "); */
	...
	public function add()
	{
		$data['action']="insert";
		$data['controller_name'] = $this->controller;
		$data['power_supply'] = Data_model::retrive('power_supply_master','*',array('delete_status'=>0),'power_supply_name','ASC');
		$data['msgName']=$this->msgName;
		$data['field']=$this->field;
		$data['primary_id']=$this->primary_id;
	...
	public function insert(Request $request)
	{
		...
		$hsn_code  = trim($request->input('hsn_code'));
		$rate  = trim(round($request->input('rate')));
		$power_value = $request->input('power_value');
		$remark = $request->input('remark');
		$data = array($this->field=>$name,'hsn_code'=>$hsn_code,'rate'=>$rate,'power_value'=>$power_value,'remark'=>$remark);
